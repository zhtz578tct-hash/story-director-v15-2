...
$('analyze').onclick=async(e)=>{e.preventDefault();const story=($('pasteText').value||$('story').value||'').trim();if(!story){alert('पहले कहानी डालें।');return;}try{const r=await fetch(API_BASE+'/api/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({story})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Analyze failed');detected=d;renderSpeakers();renderSegments();setStep(3);}catch(err){console.error(err);alert('Story analyze नहीं हो सकी: '+err.message);}};
