...
<body>
<div class="wrap">
<div class="topbar"><span class="brand">🎬 Story Director V16</span></div>
<div class="stepper"><div class="step active" data-step="1">① Story</div><div class="step" data-step="2">② Script</div><div class="step" data-step="3">③ Director</div><div class="step" data-step="4">④ Voice</div></div>

<section id="home" class="hero page">
<h1>Story → Script → Voice</h1><p>कहानी लिखें या अपनी कहानी डालें — फिर speakers, emotions और natural voices के साथ पूरा audio तैयार करें।</p>
<div class="choice-grid">
<button class="choice" id="newStory"><b>✨ नई कहानी बनाएँ</b><span>AI से नई story / screenplay तैयार करें।</span></button>
<button class="choice" id="pasteStory"><b>📋 अपनी कहानी डालें</b><span>Existing Hindi/Bhojpuri story को सीधे Director में भेजें।</span></button>
<button class="choice" id="openProjects"><b>📁 My Projects</b><span>पहले से saved projects खोलें।</span></button>
</div></section>

<section id="storyPage" class="page">
<div class="card" id="storyCard">
<h2>💡 Story</h2>
<label>Story Idea / Prompt</label><textarea id="idea" placeholder="उदाहरण: गाँव की बारिश वाली शाम में दो पुराने दोस्त कई साल बाद मिलते हैं।"></textarea>
...
<label>🎯 Extra Direction</label><input id="direction" placeholder="जैसे: 5 scenes, strong ending, Banarasi tone">
<label>🎬 Output Format</label><select id="outputFormat"><option value="normal">Normal Story</option><option value="dialogue-heavy">Dialogue Heavy — 70–80% Dialogue</option><option value="dialogue-dominant" selected>Dialogue Dominant — 80–90% Dialogue</option><option value="pure-dialogue">Pure Dialogue Scene</option><option value="cinematic-script">Cinematic Script</option></select>
<details class="advanced"><summary><b>⚙️ Advanced Settings</b></summary><div class="small" style="margin-top:10px">Advanced controls आगे के backend tuning के लिए reserve हैं। अभी मुख्य workflow को सरल रखा गया है।</div></details>
<button class="primary" id="generateStory" style="margin-top:15px">✨ Generate Story</button><div id="storyStatus" class="status"></div>
</div>

<div class="card" id="pasteCard">
<h2>📋 Paste Story</h2><label>अपनी पूरी कहानी / script</label><textarea id="pasteText" placeholder="यहाँ अपनी existing कहानी paste करें…"></textarea>
<div class="grid"><div><label>Language / Dialect</label><select id="pasteLanguage"><option>Hindi</option><option>Bhojpuri</option><option>Purvanchali / Banarasi</option><option>Hindi + Bhojpuri mix</option><option>English</option></select></div><div><label>Processing Mode</label><select id="processMode"><option value="full">🎬 Full Director Mode</option><option value="dialogue">🎙️ Dialogue + Narration</option><option value="dialogue-only">💬 Dialogue Only</option></select></div></div>
<button class="primary" id="usePaste" style="margin-top:15px">🎭 Analyze This Story</button>
</div>

<div class="card" id="scriptCard"><div class="section-title"><h2>📖 Script</h2><span class="pill" id="scriptState">Draft</span></div><label>Title</label><input id="title" placeholder="Story title"><label>Story / Script</label><textarea id="story" placeholder="Generated या pasted story यहाँ आएगी…"></textarea>
...
<div class="card"><h2>🎭 Director</h2><div class="check"><input type="checkbox" id="emotion" checked><span>Automatic Emotion Detection</span></div><div class="check"><input type="checkbox" id="adult"><span>🔞 21+ Mature Mode</span></div><button class="primary" id="analyzeStory">🎭 Detect Speakers & Emotions</button><div id="status" class="status"></div></div>
<div class="card"><div class="section-title"><h2>🎙️ Speaker Manager</h2><span class="pill" id="speakerCount">0 speakers</span></div><div id="speakers" class="empty">अभी speakers detect नहीं हुए हैं।</div></div>
<div class="card"><div class="section-title"><h2>📝 Segments</h2><span class="small">हर line को edit/preview कर सकते हैं</span></div><div id="segments" class="empty">Analysis के बाद segments यहाँ आएँगे।</div></div>
</section>

<section id="voicePage" class="page hidden">
<div class="card"><h2>🎧 Voice Studio</h2><div class="small">पहले Director में analysis पूरा करें। फिर individual segment preview या full audio बनाएं।</div><div class="toolbar" style="margin-top:12px"><button class="secondary" id="previewAllVoices">🔊 Preview Selected Voices</button><button class="secondary" id="resumeAudio" disabled>▶️ Resume</button></div><button class="primary" id="generate" disabled style="margin-top:12px">🎙️ Generate Full Audio</button><div id="progress" class="status"></div><div class="audio-box"><b>Final Audio</b><audio id="player" controls style="display:none"></audio><div class="row"><button class="secondary" id="download" disabled>⬇️ Download WAV</button><button class="secondary" id="downloadMp3" disabled>🎵 MP3</button><button class="secondary" id="downloadM4a" disabled>📱 M4A</button></div><div class="small" id="formatNote" style="margin-top:8px">WAV अभी उपलब्ध है। MP3/M4A के लिए backend encoder अगला चरण होगा; हम fake file नहीं बनाएँगे।</div></div></div>
</section>

<section id="projectsPage" class="page hidden"><div class="card"><h2>📁 My Projects</h2><div class="small">Projects इस iPhone/browser में localStorage में save होते हैं।</div><div id="projects" style="margin-top:12px"></div></div></section>
</div>

<nav class="nav"><button data-nav="home" class="active">🏠<br>Home</button><button data-nav="storyPage">✍️<br>Story</button><button data-nav="directorPage">🎭<br>Director</button><button data-nav="voicePage">🎙️<br>Voice</button><button data-nav="projectsPage">📁<br>Projects</button></nav>
<script>
...
function saveProject(){const p=projectData();const all=JSON.parse(localStorage.getItem("storyDirectorV16Projects")||"[]");const id=p.title+"|"+p.updated;all.unshift({id,title:p.title,updated:p.updated,data:p});localStorage.setItem("storyDirectorV16Projects",JSON.stringify(all.slice(0,30)));status("scriptStatus","💾 Project save हो गया ✓","ok");renderProjects()}
...
$('generateStory').onclick=async()=>{const idea=$('idea').value.trim();if(!idea){status('storyStatus','पहले Story Idea लिखिए।','err');return}const b=$('generateStory');b.disabled=true;b.textContent='⏳ कहानी बन रही है...';status('storyStatus','AI कहानी तैयार कर रहा है…','info');try{const r=await fetch('/api/story',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:idea,language:$('language').value,genre:$('genre').value,length:$('length').value,tone:$('style').value,mature:$('ageMode').value==='mature',direction:$('direction').value,outputFormat:$('outputFormat').value})});const d=await r.json().catch(()=>({}));if(!r.ok||!d.ok)throw Error(d.error||('Server error: '+r.status));const generated=typeof d.story==='string'?d.story:JSON.stringify(d.story||'',null,2);$('story').value=generated;$('title').value=(generated.split('\n').find(x=>x.trim())||'Generated Story').replace(/^TITLE\s*:\s*/i,'').trim();$('downloadStory').disabled=false;$('scriptState').textContent='Generated';status('storyStatus','✅ कहानी तैयार हो गई। अब Script step से Director पर जाएँ।','ok');setStep(2)}catch(e){status('storyStatus','❌ '+e.message,'err')}finally{b.disabled=false;b.textContent='✨ Generate Story'}};
$('usePaste').onclick=()=>{const t=$('pasteText').value.trim();if(!t){status('storyStatus','पहले कहानी paste कीजिए।','err');return}syncPaste();setStep(2);setTimeout(()=>{$('analyzeStory').click()},350)};
...
$('clearAll').onclick=()=>{if(!confirm('पूरा current project साफ़ करें?'))return;['idea','title','story','direction','pasteText'].forEach(x=>$(x).value='');detected=null;finalBlob=null;audioGeneration={running:false,paused:false,index:0,parts:[],fmt:null};$('downloadStory').disabled=true;$('generate').disabled=true;$('speakers').innerHTML='अभी speakers detect नहीं हुए हैं।';$('segments').innerHTML='Analysis के बाद segments यहाँ आएँगे।';$('player').style.display='none';$('download').disabled=true;$('downloadMp3').disabled=true;$('downloadM4a').disabled=true;setStep(1);status('scriptStatus','साफ़ कर दिया गया।','ok')};
$('analyzeStory').onclick=async()=>{const story=$('story').value.trim();if(!story){status('status','पहले कहानी लिखिए या Paste Story करें।','err');setStep(2);return}const b=$('analyzeStory');b.disabled=true;b.textContent='⏳ Analysis...';status('status','Speakers और emotions पहचाने जा रहे हैं…','info');try{const r=await fetch('/api/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({story,language:$('language').value,emotion:$('emotion').checked,mature:$('adult').checked})});const d=await r.json().catch(()=>({}));if(!r.ok||!d.ok)throw Error(d.error||('Server error: '+r.status));detected=d;renderSpeakers();renderSegments();$('generate').disabled=!(d.segments&&d.segments.length);status('status','✅ Speaker + emotion detection complete.','ok');setStep(3)}catch(e){status('status','❌ '+e.message,'err')}finally{b.disabled=false;b.textContent='🎭 Detect Speakers & Emotions'}};
...
async function previewVoice(voice,button,text){const old=button.textContent;button.disabled=true;button.textContent='⏳ सुन रहे हैं...';try{const r=await fetch('/api/tts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:text||'नमस्कार। यह इस आवाज़ का परीक्षण है।',voice,emotion:'neutral',intensity:20,delivery:'calm',pace:'normal',pause:'none',language:$('language').value})});if(!r.ok){const d=await r.json().catch(()=>({}));throw Error(d.error||('Preview error '+r.status))}const blob=await r.blob(),url=URL.createObjectURL(blob),audio=new Audio(url);audio.onended=()=>URL.revokeObjectURL(url);await audio.play()}catch(e){status('status','❌ Voice preview: '+e.message,'err')}finally{button.disabled=false;button.textContent=old}}
function renderSpeakers(){const list=detected?.speakers||[];$('speakerCount').textContent=list.length+' speakers';$('speakers').innerHTML=list.length?list.map((s,idx)=>{const segs=(detected.segments||[]).filter(x=>x.speaker===s.name);const strongest=segs.reduce((best,x)=>!best||(x.intensity??0)>(best.intensity??0)?x:best,null);const emotion=strongest?.emotion||'neutral',intensity=strongest?.intensity??50,delivery=strongest?.delivery||'conversational',voice=s.voice||defaultVoice(s);return '<div class="speaker"><div class="speaker-head"><div><div class="speaker-name">'+esc(s.name)+'</div><div class="small">'+esc(s.role||'character')+' · '+esc(s.gender||'unknown')+'</div></div><span class="pill">'+esc(emotion)+' · '+intensity+'/100</span></div><label>🎙️ Voice</label><select class="speakerVoice" data-speaker="'+esc(s.name)+'">'+voiceOption('alloy','Alloy — Natural',voice)+voiceOption('onyx','Onyx — Male',voice)+voiceOption('nova','Nova — Female',voice)+voiceOption('shimmer','Shimmer — Soft',voice)+voiceOption('echo','Echo — Deep',voice)+voiceOption('fable','Fable — Expressive',voice)+'</select><div class="row"><button type="button" class="secondary voicePreview" data-voice="'+esc(voice)+'">▶️ Preview Voice</button></div><div class="grid"><div><label>🎭 Default Emotion</label><select class="speakerEmotion" data-speaker="'+esc(s.name)+'"><option>neutral</option><option>happy</option><option>sad</option><option>angry</option><option>romantic</option><option>playful</option><option>tense</option><option>serious</option><option>concerned</option><option>dramatic</option></select></div><div><label>Intensity</label><div class="range-row"><input class="speakerIntensity" data-speaker="'+esc(s.name)+'" type="range" min="0" max="100" value="'+intensity+'"><output>'+intensity+'</output></div></div></div><div class="small" style="margin-top:9px">Delivery: '+esc(delivery)+' · Voice lock: character की voice पूरी कहानी में यही रहेगी।</div></div>'}).join(''):'<div class="empty">कोई speaker नहीं मिला।</div>';
...
function renderSegments(){const segs=detected?.segments||[];$('segments').innerHTML=segs.length?segs.map((s,i)=>'<div class="seg"><div class="who">'+esc(s.speaker||'Narrator')+'</div><div class="meta">🎭 '+esc(s.emotion||'neutral')+' · '+Number(s.intensity??50)+'/100 · '+esc(s.delivery||'conversational')+'</div><textarea class="segmentText" data-index="'+i+'">'+esc(s.text||'')+'</textarea><div class="row"><button class="secondary segmentPreview" data-index="'+i+'">▶️ Preview Segment</button><button class="secondary segmentSave" data-index="'+i+'">✓ Apply Edit</button></div></div>').join(''):'<div class="empty">Analysis के बाद segments यहाँ आएँगे।</div>';document.querySelectorAll('.segmentPreview').forEach(b=>b.onclick=()=>previewSegment(Number(b.dataset.index),b));document.querySelectorAll('.segmentSave').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.index),t=document.querySelector('.segmentText[data-index="'+i+'"]').value;detected.segments[i].text=t;status('status','Segment '+(i+1)+' update हो गया ✓','ok')})}
async function previewSegment(i,b){const s=detected?.segments?.[i];if(!s)return;const speaker=(detected.speakers||[]).find(x=>x.name===s.speaker)||{};const voice=s.voice||speaker.voice||defaultVoice(speaker);const old=b.textContent;b.disabled=true;b.textContent='⏳ Preview...';try{const r=await fetch('/api/tts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:s.text,voice,emotion:s.emotion||'neutral',intensity:s.intensity??50,delivery:s.delivery||'natural',pace:s.pace||'normal',pause:s.pause||'none',emphasis:s.emphasis||'normal',language:s.role==='narrator'?'Hindi':$('language').value})});if(!r.ok){const d=await r.json().catch(()=>({}));throw Error(d.error||('TTS error '+r.status))}const blob=await r.blob(),url=URL.createObjectURL(blob),a=new Audio(url);a.onended=()=>URL.revokeObjectURL(url);await a.play()}catch(e){status('status','❌ Segment preview: '+e.message,'err')}finally{b.disabled=false;b.textContent=old}}
...
function cleanSegmentForTTS(raw,speakerName,isNarrator){let t=String(raw??'').trim();if(!t)return '';if(isNarrator)return t.replace(/^\s*(?:Narrator|नैरेटर|वर्णन|Narration)\s*[:：-]\s*/i,'').trim();let previous='';while(t!==previous){previous=t;t=t.replace(/^\s*(?:speaker|emotion|भाव|भावना|delivery|अंदाज़|डिलीवरी|intensity|तीव्रता|pace|गति|pause|विराम|emphasis|जोर)\s*[:：-]\s*[^\n]+\s*/i,'').replace(/^\s*(?:\[[^\]]{1,200}\]|\([^)]{1,200}\))\s*/g,'').trim();if(speakerName){const n=String(speakerName).trim(),lower=t.toLowerCase(),nl=n.toLowerCase();if(lower.startsWith(nl+':')||lower.startsWith(nl+'：')||lower.startsWith(nl+'-'))t=t.slice(n.length+1).trim()}}t=t.replace(/^\s*[^\n:：]{1,40}\s*[:：]\s*/u,'').trim();return t.replace(/^\s*[“”"']\s*/, '').replace(/\s*[“”"']\s*$/i,'').trim()}
async function generateAudioFrom(index=0){if(!detected?.segments?.length)return;const segments=detected.segments;audioGeneration.running=true;audioGeneration.paused=false;$('generate').disabled=true;$('download').disabled=true;try{for(let i=index;i<segments.length;i++){audioGeneration.index=i;const s=segments[i]||{},speakerInfo=(detected.speakers||[]).find(x=>x.name===s.speaker)||{},role=String(speakerInfo.role||'').toLowerCase(),gender=String(speakerInfo.gender||'').toLowerCase(),isNarrator=role.includes('narrator')||role.includes('narration'),voice=s.voice||speakerInfo.voice||(isNarrator?'shimmer':gender==='female'?'nova':gender==='male'?'onyx':'alloy'),language=isNarrator?'Hindi':($('language').value||'Hindi'),speakerName=String(s.speaker||'').trim(),text=cleanSegmentForTTS(s.text||'',speakerName,isNarrator);if(!text)continue;status('progress','🎙️ Segment '+(i+1)+' / '+segments.length+(document.hidden?'\n⏸️ Screen/background detected — generation will resume automatically.':''),'info');let success=false,lastError=null;for(let attempt=1;attempt<=3&&!success;attempt++){try{const r=await fetch('/api/tts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text,voice,emotion:s.emotion||'neutral',emotion2:s.emotion2||'',intensity:s.intensity??50,delivery:s.delivery||'natural',pace:s.pace||'normal',pause:s.pause||'none',emphasis:s.emphasis||'normal',language})});if(!r.ok){const d=await r.json().catch(()=>({}));throw Error(d.error||('TTS error '+r.status))}const rawAudio=await r.arrayBuffer(),processedAudio=compressLongSilence(rawAudio),x=wavInfo(processedAudio);if(!x)throw Error('TTS ने valid WAV audio नहीं लौटाया।');if(!audioGeneration.fmt)audioGeneration.fmt={ch:x.ch,rate:x.rate,bits:x.bits};else if(x.ch!==audioGeneration.fmt.ch||x.rate!==audioGeneration.fmt.rate||x.bits!==audioGeneration.fmt.bits)throw Error('TTS segment का WAV format अलग मिला।');if(i>0){const prev=segments[i-1],gapMs=prev?.speaker!==s?.speaker?350:25,bytesPerSecond=audioGeneration.fmt.rate*audioGeneration.fmt.ch*audioGeneration.fmt.bits/8;audioGeneration.parts.push(new Uint8Array(Math.max(0,Math.round(bytesPerSecond*gapMs/1000))))}audioGeneration.parts.push(x.data);success=true;status('progress','🎙️ Segment '+(i+1)+' / '+segments.length+' ✓','info')}catch(e){lastError=e;if(document.hidden){audioGeneration.paused=true;status('progress','⏸️ iPhone ने background में generation रोक दी है.\nScreen वापस ON करते ही Segment '+(i+1)+' से automatically resume होगा.','info');return}if(attempt<3){status('progress','⚠️ Segment '+(i+1)+' temporarily failed.\nRetry '+attempt+'/3…','info');await new Promise(resolve=>setTimeout(resolve,1200*attempt))}}}if(!success){audioGeneration.paused=true;status('progress','❌ Segment '+(i+1)+' complete नहीं हो सका.\n'+(lastError?.message||'Unknown TTS error')+'\n\nScreen ON रखें।','err');return}}if(!audioGeneration.fmt||!audioGeneration.parts.length)throw Error('कोई audio segment तैयार नहीं हुआ।');finalBlob=wavBuild(audioGeneration.parts,audioGeneration.fmt);window.finalBlob=finalBlob;$('player').src=URL.createObjectURL(finalBlob);$('player').style.display='block';$('download').disabled=false;$('downloadMp3').disabled=true;$('downloadM4a').disabled=true;audioGeneration.running=false;audioGeneration.paused=false;status('progress','✅ पूरा WAV audio तैयार है।','ok');setStep(4)}catch(e){audioGeneration.paused=true;status('progress','❌ '+(e.message||'Audio generation failed')+'\n\nScreen ON करके दोबारा कोशिश करें.','err')}finally{audioGeneration.running=false;$('generate').disabled=false;$('resumeAudio').disabled=!audioGeneration.paused}}
function tryResumeAudio(){if(audioResumeBusy||document.hidden||!audioGeneration.paused||audioGeneration.running||!detected?.segments?.length)return;audioResumeBusy=true;clearTimeout(audioResumeTimer);audioResumeTimer=setTimeout(()=>{try{if(!document.hidden&&audioGeneration.paused&&!audioGeneration.running){status('progress','▶️ Screen वापस ON हो गया। Segment '+(audioGeneration.index+1)+' से resume हो रही है…','info');generateAudioFrom(audioGeneration.index)}}finally{audioResumeBusy=false}},700)}
...
$('generate').onclick=async()=>{if(!detected?.segments?.length)return;audioGeneration={running:false,paused:false,index:0,parts:[],fmt:null};finalBlob=null;await generateAudioFrom(0)};$('resumeAudio').onclick=()=>tryResumeAudio();$('download').onclick=()=>{if(finalBlob)downloadBlob(finalBlob,nameSafe($('title').value||'story')+'-audio.wav')};$('downloadMp3').onclick=()=>status('progress','MP3 export अभी backend encoder के बिना उपलब्ध नहीं है। WAV को source master की तरह रखा गया है।','info');$('downloadM4a').onclick=()=>status('progress','M4A export अभी backend encoder के बिना उपलब्ध नहीं है।','info');$('previewAllVoices').onclick=()=>{const s=detected?.speakers?.[0];if(!s)return status('progress','पहले speakers detect करें।','err');const b=document.querySelector('.voicePreview');if(b)previewVoice(s.voice||defaultVoice(s),b)};
...
async function analyze(env,b){
 ...
 const instructions=[
  ...
  "For character dialogue, remove leading labels such as \"रवि:\", \"नेहा:\", \"Emotion:\", \"Delivery:\" and remove leading parenthetical stage directions. Keep those details only in JSON metadata fields.",
